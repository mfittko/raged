...
async def test_get_entity_neighborhood(mock_driver):
    ...
    mock_record = {
        "e": MagicMock(get=lambda k, d=None: {"name": "TestClass", "type": "class", "description": "Test", "mentionCount": 5}.get(k, d)),
        "connections": [
            MagicMock(get=lambda k, d=None: {"name": "OtherClass", "type": "class"}.get(k, d))
        ],
        "documents": [
            MagicMock(get=lambda k, d=None: {"id": "doc-1", "docType": "code", "source": "test.py"}.get(k, d))
        ]
    }
...
async def test_search_entities(mock_driver):
    ...
    mock_entities = [
        MagicMock(get=lambda k, d=None, i=i: {
            "name": f"Entity{i}",
            "type": "class",
            "description": "Test",
            "mentionCount": 5 - i
        }.get(k, d)) for i in range(3)
    ]
