...
{f'Context: {context}' if context else ''}"""
    ...
    async def _extract_with_tools(self, prompt: str, schema: Dict, tool_name: str, model: str) -> Dict:
        ...
        if "objects" in text.lower() or "visible" in text.lower():
            ...
            for line in lines:
                if "object" in line.lower() or "visible" in line.lower():
                    ...
                    words = line.split()
                    result["detected_objects"].extend([w.strip(",.:-") for w in words if len(w) > 3])
