...
def get_driver() -> AsyncDriver:
    ...
    if _driver is None:
        ...
        _driver = AsyncGraphDatabase.driver(
            NEO4J_URL, auth=(NEO4J_USER, NEO4J_PASSWORD) if NEO4J_PASSWORD else None
        )
    return _driver
...
async def upsert_document(
    doc_id: str, doc_type: str, source: str, collection: str, summary: str = ""
) -> None:
...
async def get_entity_neighborhood(name: str, depth: int = 2) -> Dict:
    ...
    async with driver.session() as session:
        ...
        if not record:
            return {"entity": None, "connections": [], "documents": []}
        ...
        return {
            ...
            "connections": [
                {"name": c.get("name"), "type": c.get("type")} for c in connections if c
            ],
            "documents": [
                {
                    "id": d.get("id"),
